    ...
    <div className='space-y-8'>
      ...
      <section>
        <div className='mb-4 flex items-center justify-between'>
          <h2 className='text-tsinelas-text-strong text-xl font-bold'>Overview</h2>
          <Button
            variant='outline'
            size='sm'
            leftIcon={
      ...
      {/* Quick Actions */}
      <section>
        <h2 className='text-tsinelas-text-strong mb-4 text-xl font-bold'>
          Quick Actions
        </h2>
          ...
          {quickActions.map(action => (
            ...
            >
              ...
              <Link to={action.link} className='px-4'>
                <ArrowRight className='text-tsinelas-text-disabled h-5 w-5' />
              </Link>
            </Card>
          ))}
        </div>
          ...
          <CardContent className='p-0'>
            {activityLoading ? (
              <div className='flex items-center justify-center py-6'>
                <RefreshCw className='text-tsinelas-text-disabled h-6 w-6 animate-spin' />
              </div>
            ) : recentActivity.length === 0 ? (
              <div className='text-tsinelas-text-disabled py-6 text-center'>
                <CheckCircle className='text-tsinelas-text-disabled mx-auto mb-2 h-8 w-8' />
              ...
              <div className='divide-y divide-slate-100'>
                {recentActivity.map(item => (
                  <div
                    key={item.id}
                    className='hover:bg-tsinelas-bg-surface-raised flex items-start gap-4 p-4'
                  >
                    ...
                    <div className='min-w-0 flex-1'>
                      <div className='mb-1 flex flex-wrap items-center gap-2'>
                        ...
                        <span className='text-tsinelas-text-strong text-sm font-medium'>
                          {item.issue_type.replace(/_/g, ' ')}
                        </span>
                      </div>
                      {item.description && (
                        <p className='text-tsinelas-text-support mb-1 line-clamp-2 text-sm'>
                          {item.description}
                        </p>
                      )}
                      {item.document && (
                        <Link
                          ...
                          className='text-tsinelas-text-brand mb-1 inline-flex items-center gap-1 text-sm font-medium hover:underline'
                        >
                          ...
                          <span className='text-tsinelas-text-support ml-1 line-clamp-1 font-normal'>
                            - {item.document.title}
                          </span>
                        </Link>
                      )}
                      {item.resolution && (
                        <p className='text-tsinelas-text-disabled text-sm italic'>
                          &quot;{item.resolution}&quot;
                        </p>
                      )}
                      <div className='text-tsinelas-text-disabled mt-1 flex flex-wrap items-center gap-3 text-xs'>
